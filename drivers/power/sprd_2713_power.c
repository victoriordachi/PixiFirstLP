...
//add by tt modify temperature do not change 20150207
int temperature = 0;
//end add by tt 20150207
	...
	case SAVE_CAPACITY:
		{
			...
			SPRDBAT_DEBUG("SAVE_CAPACITY+++++battery capacity :%d\n", sprdbat_data->bat_info.capacity);
...
static int plugin_callback(int usb_cable, void *data)
{
	...
	if (sprdbat_data->bat_info.adp_type == ADP_TYPE_DCP)
 	...
 	SPRDBAT_DEBUG("plugin_callback:sprdbat_data->bat_info.capacity:%d\n",
		      sprdbat_data->bat_info.capacity);
...
static void sprdbat_update_capacty(void)
{
	...
	case POWER_SUPPLY_STATUS_DISCHARGING:
		if(sprdbat_data->bat_info.capacity >= 15)
		{
			sprdbat_data->pdata->cap_one_per_time = 60;
		}
		else
		{
...
//add by tt modify temperature do not change 20150207
	//printk("sprdbat_dat->bat_info.cur_temp = %d\n",sprdbat_data->bat_info.cur_temp);
	if((sprdbat_data->bat_info.cur_temp - temperature) >= 15 || (sprdbat_data->bat_info.cur_temp - temperature) <= -15)
	{
		power_supply_changed(&sprdbat_data->battery);
		temperature = sprdbat_read_temp();
	}
//end add by tt 20150207
