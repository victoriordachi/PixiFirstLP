...
//add flash hardware info
    	{
		#include <linux/hardware_info.h>
		char emmc_device_name[HARDWARE_MAX_ITEM_LONGTH] = "unknow";
		printk("card->cid.manfid = %x\n",card->cid.manfid);     
        	if(0x000011 == card->cid.manfid)
		{
			strcpy(emmc_device_name, "Toshiba TYC0FH121638RA");
			hardwareinfo_set_prop(HARDWARE_FLASH,emmc_device_name);
        	}
		else if(0x000070 == card->cid.manfid)
		{
                	strcpy(emmc_device_name, "Kingston 04EMCP04-NL2AS100");
			hardwareinfo_set_prop(HARDWARE_FLASH,emmc_device_name);
        	}
		else if(0x000090 == card->cid.manfid)
		{
                	strcpy(emmc_device_name, "hynix H9TP32A4GDDCPR-KGM");
			hardwareinfo_set_prop(HARDWARE_FLASH,emmc_device_name);
        	}
		else if(0x000015 == card->cid.manfid)
		{
                	strcpy(emmc_device_name, "Samsung KMNJ2000ZM-B207");
			hardwareinfo_set_prop(HARDWARE_FLASH,emmc_device_name);
        	}
		else
		{
                	strcpy(emmc_device_name, "please add falsh name");
			hardwareinfo_set_prop(HARDWARE_FLASH,emmc_device_name);
		}
	}
//end add flash hardware info
	return 0;
