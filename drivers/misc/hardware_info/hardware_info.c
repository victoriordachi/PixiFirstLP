...
#ifdef WT_FTM_FLASH
//char emmc_device_name[HARDWARE_MAX_ITEM_LONGTH];//wingtech wangfuqiang add
#endif
...
static long hardwareinfo_ioctl(struct file *file, unsigned int cmd,unsigned long arg)
{
	...
	case HARDWARE_BT_GET:
		hardwareinfo_set_prop(HARDWARE_BT, "SR2351");
		hardwareinfo_num = HARDWARE_BT;
		break;
	case HARDWARE_WIFI_GET:
		hardwareinfo_set_prop(HARDWARE_WIFI, "SR2351");
	...
	case HARDWARE_GPS_GET:
		hardwareinfo_set_prop(HARDWARE_GPS, "SR2351");
	    hardwareinfo_num = HARDWARE_GPS;
		break;
	case HARDWARE_FM_GET:
		hardwareinfo_set_prop(HARDWARE_FM, "SR2351");
	    hardwareinfo_num = HARDWARE_FM;		
		break;
	case HARDWARE_SDCARD_STATUS:
	    hardwareinfo_num = HARDWARE_SDSTATUS;
		break;
	case HARDWARE_FRONT_CAMERA_SET:
		if (copy_from_user(hardwareinfo_name[HARDWARE_FRONT_CAM],data,strlen(data)+1))
		{
			printk("%s, copy front camera info from usr error\n", __func__);
			ret =  -EINVAL;
		}
		//hardwareinfo_set_prop(HARDWARE_FRONT_CAM, char *name);
		return 0;
		break;
	case HARDWARE_BACK_CAMERA_SET:
		if (copy_from_user(hardwareinfo_name[HARDWARE_BACK_CAM],data,strlen(data)+1))
		{
			printk("%s, copy back camera info from usr error\n", __func__);
			ret =  -EINVAL;
		}
		//hardwareinfo_set_prop(HARDWARE_BACK_CAM, char *name);
		return 0;
		break;
	default:
		ret = -EINVAL;
