...
static unsigned int alarm_boot = 0;
static int sprdfb_dispc_is_alarm_boot_check(char *str)
{
	if (strncmp(str, "alarm", 5) == 0)
	{
		alarm_boot = 1;
	}
	pr_debug("%s: alarm_boot: %d\n", __func__, alarm_boot);

	return alarm_boot;
}
__setup("androidboot.mode=", sprdfb_dispc_is_alarm_boot_check);
...
DONE:
	if (!alarm_boot || (type != SPRDFB_DYNAMIC_MIPI_CLK)) {
