...
//add camera adb debug by tt 20150304
#include <linux/proc_fs.h>
//end add camera adb debug by tt 20150304
...
sensor_k_writei2c_return:
	...
	if (ret) {
		SENSOR_PRINT_ERR("sensor w done, ret %d \n", ret);
	}
	return ret;
}
...
sensor_k_readi2c_return:
	...
	if (ret) {
		SENSOR_PRINT_ERR("_Sensor_K_ReadI2C, ret %d \n", ret);
	}
	return ret;
}
...
//写寄存器命令:
//echo reg_addr reg_value bit > camsensor

//reg_addr与reg_value是16进制数据

//bit=0 reg_addr 8位 reg_value 8位
//bit=1 reg_addr 8位 reg_value 16位
//bit=2 reg_addr 16位 reg_value 8位
//bit=3 reg_addr 16位 reg_value 16位

/////////////////////////////////////////////////////////////////////////////////
//add camera adb debug by tt 20150304
static int camera_hw_debug_read_reg(char *page, char **start, off_t off, int count, int *eof, void *data)
{
	return count;
}
static int camera_hw_debug_write_reg(struct file *file, const char *buffer, unsigned long count, void *data)
{
	char regBuf[64] = {'\0'};
	SENSOR_REG_BITS_T_PTR pReg;
	
	pReg = kzalloc(sizeof(SENSOR_REG_BITS_T), GFP_KERNEL);
	int copyBufSize = (count < (sizeof(regBuf) - 1) ? (count) : sizeof(regBuf - 1));
	if(copy_from_user(regBuf, buffer, copyBufSize))
	{
		return -EFAULT;
	}
	printk("tttt1\n");
	if(sscanf(regBuf, "%x %x %d", &pReg->reg_addr, &pReg->reg_value, &pReg->reg_bits) == 3)
	{
		printk("ttt1 i2cAddr=%x, addr=%x, value=%x, bit=%d \n", s_p_sensor_mod->cur_i2c_client->addr, pReg->reg_addr, pReg->reg_value, pReg->reg_bits);
		_Sensor_K_WriteReg(pReg);
	}
	else if(sscanf(regBuf, "%x %d", &pReg->reg_addr, &pReg->reg_bits) == 2)
	{
		_Sensor_K_ReadReg(pReg);
		printk("ttt2 i2cAddr=%x, addr=%x, value=%x, bit=%d \n", s_p_sensor_mod->cur_i2c_client->addr, pReg->reg_addr, pReg->reg_value, pReg->reg_bits);
		
	}
	return copyBufSize;
}
static const struct file_operations cam_hw_debug_ops = {
	.owner = THIS_MODULE,
	.read = camera_hw_debug_read_reg,
	.write = camera_hw_debug_write_reg,
};
...
//add camera adb debug by tt 20150304
	pEntry = proc_create("driver/camsensor", 0666, NULL, &cam_hw_debug_ops);
	if(pEntry == NULL)
	{
		printk("add /proc/dirver/camsensor entry fail\n");
	}
//end add camera adb debug by tt 20150304
	return 0;
}
...
//eng add camera adb debug by tt 20150304
