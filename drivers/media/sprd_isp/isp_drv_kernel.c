...
int _isp_kernel_proc(void *arg)
{
	...
	while(1) {
		//printk("isp_k: _isp_kernel_proc \n");
		if (0 == down_interruptible(&dev->kernel_sem)) {
			//printk("isp_k: _isp_kernel_proc, down \n");
...
static int32_t _isp_lnc_param_load(struct isp_reg_bits *reg_bits_ptr, uint32_t counts)
{
	...
	if((0x00!=s_isp_alloc_addr)
		...
		while ((0x00 == (reg_value&ISP_INT_LENS_LOAD)) && (time_out_cnt < ISP_TIME_OUT_MAX*1000)) {                      //jacky
			udelay(1);
			reg_value=ISP_READL(ISP_INT_RAW);
			time_out_cnt++;
		}
		if (time_out_cnt >= ISP_TIME_OUT_MAX*1000) {       //jacky
			ret = -1;
...
static irqreturn_t _isp_irq_root(int irq, void *dev_id)
{
	...
	if ((ISP_INT_LENS_LOAD & irq_line) == ISP_INT_LENS_LOAD) {
		//printk("isp_k: _isp_irq_root, irq_line 0x%x \n", irq_line);
		...
		switch (cmd)
		{
			...
			case ISP_IO_LNC: {
				...
				ret = _isp_lnc_param_load(reg_bits_ptr, reg_param.counts);
					...
					//printk("isp_k: befor load, 0x%x: 0x%x, 0x%x, counts %d \n", reg_bits_ptr, reg_bits_ptr->reg_addr, reg_bits_ptr->reg_value, reg_param.counts);
					ret = down_timeout(&g_isp_dev_ptr->lnc_load.load_done_sem,  msecs_to_jiffies(500));
					//printk("isp_k: after load \n");
				}
				if (unlikely(ret)) {
