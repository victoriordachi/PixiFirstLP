...
{
#ifndef CONFIG_LEDS_SPRD_BLTC_RGB_MIX
...
#ifdef CONFIG_LEDS_SPRD_BLTC_RGB_MIX
static void sprd_leds_bltc_rgb_mix_disable(struct sprd_leds_bltc_rgb *brgb)
{
...
static void sprd_leds_bltc_rgb_shutdown(struct platform_device  *dev)
{
        ...
        sprd_leds_bltc_rgb_disable(brgb);
#ifdef CONFIG_LEDS_SPRD_BLTC_RGB_MIX
        sprd_leds_bltc_rgb_mix_disable(brgb);
#endif
        mutex_unlock(&brgb->mutex);
}
...
#ifdef CONFIG_LEDS_SPRD_BLTC_RGB_MIX
        sprd_bltc_rgb_init(platform_get_drvdata(dev));
#endif
